...
> **`--no-cpu-throttling` обязателен.** Библиотека отвечает Telegram сразу, а
> обновление обрабатывает в фоне. По умолчанию Cloud Run отбирает процессор,
> как только запрос закрыт, и фоновая обработка застывает на полуслове: бот
> отвечает «читаю бollettу» и молчит, в логах — только `POST 200` без ошибок.
>
> `--memory 1Gi` — с запасом под разбор PDF, 512Mi впритык.

Оба флага задаются при деплое и в коде не хранятся, так что при переезде или
пересоздании сервиса их легко потерять.
